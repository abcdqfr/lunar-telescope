...
static int parse_application(json_object *obj, telescope_application_t *app) {
  ...
  /* Ensure counters/pointers are initialized even if the caller didn't
   * zero-init. */
  app->args = NULL;
  app->args_count = 0;
  app->env = NULL;
  ...
  if (json_object_object_get_ex(obj, "env", &env_obj)) {
    /* Prefer the json-c native length API (avoids double-iteration surprises
     * and analyzer false positives). */
...
static int parse_observability(json_object *obj,
                               telescope_observability_t *obs) {
  json_object *tmp;
...
int telescope_config_load(const char *config_path,
                          struct telescope_config **config_out) {
  json_object *root;
  json_object *tmp;

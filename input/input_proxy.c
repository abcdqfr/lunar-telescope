...
int input_proxy_create(bool enable_prediction, uint32_t prediction_window_ms,
                       bool enable_scroll_smoothing,
  ...
  if (enable_prediction) {
    ...
    proxy->rust_predictor = rust_input_predictor_create(
        prediction_window_ms, 0.7, /* smoothing_factor */
        0.9                        /* velocity_decay */
    );
...
int input_proxy_process(struct input_proxy *proxy,
  ...
  if (event->type == INPUT_EVENT_SCROLL && proxy->enable_scroll_smoothing &&
      proxy->scroll_smoother) {
    double smoothed_dx, smoothed_dy;
    if (scroll_smoother_process(proxy->scroll_smoother, event->scroll.dx,
                                event->scroll.dy, event->scroll.discrete,
  ...
  if (event->type == INPUT_EVENT_POINTER_MOTION && proxy->enable_prediction) {
    ...
    if (predicted) {
      *predicted = *event;
      ...
      /* Use Rust predictor if available, otherwise fallback to simple
       * prediction */
      if (proxy->use_rust_predictor && proxy->rust_predictor) {
        ...
        int ret = rust_input_predictor_predict_pointer(
            proxy->rust_predictor, timestamp_sec, event->pointer_motion.dx,
            event->pointer_motion.dy, &predicted_dx, &predicted_dy);
      ...
      struct pending_prediction *pending =
          malloc(sizeof(struct pending_prediction));
      if (pending) {
        pending->frame_id = frame_id;
        /*
         * Ownership rule:
         * - If caller requested predicted_out, the caller owns that returned
         * event and may free it. We therefore store an internal copy for
         * reconciliation.
         * - If caller did not request predicted_out, the proxy owns the
         * predicted event and stores it directly.
         */
        if (predicted_out) {
...
int input_proxy_reconcile(struct input_proxy *proxy, uint64_t frame_id,
  ...
  if (found) {
    ...
    if (!prediction_correct && actual_event && found->predicted_event) {
      ...
      if (found->predicted_event->type == INPUT_EVENT_POINTER_MOTION &&
        ...
        /* In production, this would trigger a correction event to the
         * compositor */

    ...
    if (event->type == INPUT_EVENT_POINTER_MOTION && proxy->enable_prediction) {
        ...
        if (predicted) {
            *predicted = *event;
            ...
            /*
             * C-only predictor (deterministic, minimal moving parts):
             * Simple extrapolation assuming near-constant velocity over a short window.
             * This keeps the hot path C-first and avoids requiring a second toolchain.
             */
